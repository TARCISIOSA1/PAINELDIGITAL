...
export default function CadastroSessao() {
  ...
  // --- Campos para oradores da tribuna ---
  const [tempoFalaDefault, setTempoFalaDefault] = useState(300);

  function adicionarOrador(id) {
    if (!id || tribuna.some(o => o.id === id)) return;
    const p = vereadores.find(p => p.id === id);
    if (p) setTribuna([
      ...tribuna,
      { id: p.id, nome: p.nome, partido: p.partido, tempoFala: tempoFalaDefault }
    ]);
  }
      ...
      case "Votação":
        return (
          <div>
           ...
           case "Tribuna":
        // Bloco de seleção e ordem dos oradores da tribuna
        return (
          <div>
            <h4 style={{ marginBottom: 10 }}>Oradores da Tribuna</h4>
            <div style={{ marginBottom: 14, display: 'flex', alignItems: 'center', gap: 16 }}>
              ...
              <select onChange={e => adicionarOrador(e.target.value)} value="">
                ...
                {vereadores.filter(p => !tribuna.some(o => o.id === p.id)).map(p => (
            ...
            {tribuna.length > 0 &&
              <table className="tabela-oradores-tribuna" style={{ width: "100%", background: "#f7f8fa", marginBottom: 18 }}>
                <thead>
                  <tr>
                    <th>Ordem</th>
                    <th>Nome</th>
                ...
                <tbody>
                  {tribuna.map((o, idx) => (
                    <tr key={o.id}>
                      ...
                      <td>
                        <button type="button" onClick={() => removerOrador(idx)} style={{ color: "#a00" }}>Remover</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            {/* MANTÉM a lista de falas da tribuna, se houver */}
            <div><b>Tribuna — falas dos vereadores:</b></div>
            <ul>
              {tribuna && tribuna.length > 0 ? (
                tribuna.map((f, idx) => (
                  <li key={idx}>
                    {f.vereador || f.nome}: {f.fala || ""}{" "}
                    <span style={{ color: "#888" }}>{f.horario || ""}</span>
                  </li>
                ))
              ) : (
